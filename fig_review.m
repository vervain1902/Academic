...
% License: https://github.com/jmgirard/DARMA/blob/master/LICENSE.txt
    ...
    % Set minimum size
    set(handles.figure_review,'Units','normalized','Position',[0,0,1,1],'Visible','on');
    drawnow;
...
function default_XYC(handles)
    ...
    set(handles.axis_C, ...
        'YLim',[-100,100], ...
    	'YTick',linspace(-100,100,9), ...
        'YTickLabel', -2:0.5:2, ...
    	'XLim',[-100,100], ...
    	'XTick',linspace(-100,100,9), ...
        'XTickLabel',-2:0.5:2);
    handles.axis_C.YRuler.MinorTickValues = linspace(-100,100,9);
    handles.axis_C.XRuler.MinorTickValues = linspace(-100,100,9);
    % Update
    guidata(handles.figure_review,handles);
    drawnow();
end
...
function set_time_axes(handles)
    rounded_dur = ceil(handles.dur/10)*10;  % 将视频时长按秒向上取整到整10
    set(handles.axis_X, ...
        'XLim',[0,rounded_dur], ...
        'XTick',0:10:rounded_dur, ...
        'XTickLabel',0:10:rounded_dur, ...
...
function listbox_Callback(hObject,~)
    ...
    for i = 1:size(dataX,1)
        ...
        RHO = ones(1,10)*handles.mag/60;
        [X,Y] = pol2cart(THETA,RHO);
        X = X+dataX(i);
        Y = Y+dataY(i);
        h = fill(X,Y,col);
        axis square;
        set(h,'FaceAlpha',min([size(dataX,1),10])/size(dataX,1),'EdgeColor',[0,0,0]);
    end
...
function plot_centroids(hObject,~)
    ...
    set(handles.axis_C, ...
        ...
        'XTick',linspace(-1*handles.mag,handles.mag,9),'YTick',linspace(-1*handles.mag,handles.mag,9), ...
        'XTickLabel',[-2:0.5:2],'YTickLabel',[-2:0.5:2]);
    handles.axis_C.YRuler.MinorTickValues = linspace(-1*handles.mag,handles.mag,9);
    handles.axis_C.XRuler.MinorTickValues = linspace(-1*handles.mag,handles.mag,9);
    axes(handles.axis_C);
    CS = get(gca,'ColorOrder');
    for i = 1:size(handles.AllRatingsX,2)
        ...
        plot(x0,y0,'o','MarkerFaceColor',col,'MarkerEdgeColor',[0 0 0],'MarkerSize',3);
    end
    for i = 1:size(handles.AllRatingsX,2)
        dataX = handles.AllRatingsX(:,i);
        dataY = handles.AllRatingsY(:,i);
        ...
        colorindex = mod(i,7); if colorindex==0, colorindex = 7; end
        col = CS(colorindex,:);
        plot(x0,y0,'o','MarkerFaceColor',col,'MarkerEdgeColor',[0 0 0],'MarkerSize',1);
    end
    text(900,0,handles.labelX,'HorizontalAlignment','center','BackgroundColor',[1 1 1],'FontSize',12,'Margin',5,'Rotation',-90);
    text(0,900,handles.labelY,'HorizontalAlignment','center','BackgroundColor',[1 1 1],'FontSize',12,'Margin',5);
end
...
function update_plots(handles)
    ...
    if get(handles.toggle_meanplot,'Value')==get(handles.toggle_meanplot,'Min')
        ...
        plot(handles.Seconds,handles.AllRatingsX,'-','LineWidth',1.5,'ButtonDownFcn',{@axis_click_Callback,'X'});
        hold on;
        ylim([-1*handles.mag,handles.mag]);
        xlim([0,ceil(max(handles.Seconds))]);
        set(gca,'YTick',linspace(-1*handles.mag,handles.mag,9),'YTickLabel',[],'YGrid','on','TickLength',[0.005 0]);
        ylabel(handles.labelX,'FontSize',10);
        set(gca, 'XTick', 0:10:ceil(max(handles.Seconds)));
        set(handles.axis_X,'ButtonDownFcn',{@axis_click_Callback,'X'});
        ts_X = plot(handles.axis_X,[0,0],[handles.mag,-1*handles.mag],'k');
        hold off;
        % Configure second (Y) axis for normal plots
        axes(handles.axis_Y); cla;
        plot(handles.Seconds,handles.AllRatingsY,'-','LineWidth',1.5,'ButtonDownFcn',{@axis_click_Callback,'Y'});
        hold on;
        ylim([-1*handles.mag,handles.mag]);
        xlim([0,ceil(max(handles.Seconds))]);
        set(gca,'YTick',linspace(-1*handles.mag,handles.mag,9),'YTickLabel',[],'YGrid','on','TickLength',[0.005 0]);
    ...
    elseif get(handles.toggle_meanplot,'Value')==get(handles.toggle_meanplot,'Max')
        ...
        set(handles.axis_X,'ButtonDownFcn',{@axis_click_Callback,'X'});
        hold on;
        plot(handles.Seconds,handles.AllRatingsX,'-','LineWidth',1.5,'Color',[.8 .8 .8],'ButtonDownFcn',{@axis_click_Callback,'X'});
        plot(handles.Seconds,handles.MeanRatingsX,'-','LineWidth',1.5,'Color',[1 0 0],'ButtonDownFcn',{@axis_click_Callback,'X'});
        ylim([-1*handles.mag,handles.mag]);
        xlim([0,ceil(max(handles.Seconds))]);
        ...
        set(handles.axis_Y,'ButtonDownFcn',{@axis_click_Callback,'Y'});
        hold on;
        plot(handles.Seconds,handles.AllRatingsY,'-','LineWidth',1.5,'Color',[.8 .8 .8],'ButtonDownFcn',{@axis_click_Callback,'Y'});
        plot(handles.Seconds,handles.MeanRatingsY,'-','LineWidth',1.5,'Color',[1 0 0],'ButtonDownFcn',{@axis_click_Callback,'Y'});
        ylim([-1*handles.mag,handles.mag]);
        xlim([0,ceil(max(handles.Seconds))]);
        set(gca,'YTick',linspace(-1*handles.mag,handles.mag,5),'YTickLabel',[],'YGrid','on');
        ylabel(handles.labelY,'FontSize',10);
